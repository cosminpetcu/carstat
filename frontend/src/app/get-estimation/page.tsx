...
export default function CleanEstimationPage() {
  ...
  useEffect(() => {
    ...
    if (savedHistory) {
      try {
        ...
        setEstimationHistory(parsed.map((item: any) => ({
          ...item,
          timestamp: new Date(item.timestamp)
        })));
      } catch (e) {
        console.error("Failed to load estimation history:", e);
      }
    }
  }, []);

  const saveToHistory = (car: CarData, result: EstimationResult, notes?: string) => {
    const newItem: EstimationHistory = {
  ...
  const selectHistoryItem = (item: EstimationHistory) => {
    ...
    setEstimationResult(item.result);
    setCarData(item.car);
              ...
              <div className="space-y-6">
                <div className="bg-blue-50 rounded-2xl p-6">
                  ...
                  <ul className="space-y-3 text-blue-800">
                    ...
                    <li className="flex items-start">
                      <span className="w-2 h-2 bg-blue-600 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                      Save estimation history for future reference
                    </li>
                  </ul>
                </div>
                        ...
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center mt-2 space-x-4 text-sm text-gray-600">
                              ...
                              <span>{item.car.engine_capacity} cc</span>
                            </div>
                          ...
                          <div className="text-right">
                            <div className="text-2xl font-bold text-blue-600">
                              €{item.result.estimated_price.toLocaleString()}
                            </div>
                            <div className="text-sm text-gray-600">
                              {item.result.confidence_level} confidence
                            </div>
                            <div className="text-sm text-gray-600">
                              {item.result.market_position}
                            </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className={`p-6 rounded-xl border-l-4 ${
                    estimationResult.confidence_level === "High" ? "bg-green-50 border-green-500" :
                        ...
                        <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                          ...
                          <div className="flex items-center space-x-2">
                            <span className={`w-2 h-2 rounded-full ${
                              item.status === "positive" ? "bg-green-500" : "bg-gray-400"
                            }`}></span>
